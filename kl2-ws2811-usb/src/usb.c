...
/**
 * Kinetis Buffer Descriptor Table entry
...
#define USB_BD_OWN_MASK       0x80
#define USB_BD_DATA1_MASK     0x40
#define USB_BD_KEEP_MASK      0x20
#define USB_BD_NINC_MASK      0x10
#define USB_BD_DTS_MASK       0x08
#define USB_BD_BDT_STALL_MASK 0x04
#define USB_BD_TOK_PID_SHIFT  2
#define USB_BD_TOK_PID_MASK   (0xF << (USB_BD_TOK_PID_SHIFT))
#define USB_BD_TOK_PID(N)     (((N) & 0xF) << (USB_BD_TOK_PID_SHIFT))
#define USB_BD_BC_SHIFT       16
#define USB_BD_BC_MASK        (0x3FF << (USB_BD_BC_SHIFT))
#define USB_BD_BC(C)          (((C) & 0x3FF) << (USB_BD_BC_SHIFT))

#define USB_BD_TOK_PID_SETUP  USB_BD_TOK_PID(0xD)
#define USB_BD_TOK_PID_OUT    USB_BD_TOK_PID(0x1)
#define USB_BD_TOK_PID_IN     USB_BD_TOK_PID(0x9)
#define USB_BD_TOK_PID_DATA0  USB_BD_TOK_PID(0x3)
#define USB_BD_TOK_PID_DATA1  USB_BD_TOK_PID(0xB)

#define USB_BD_DESC(count, data) (USB_BD_BC(count) | USB_BD_OWN_MASK |\
        ((data) ? USB_BD_DATA1_MASK : 0) | USB_BD_DTS_MASK)

_Static_assert(sizeof(USBBufferDescriptor) == 8, "Buffer Descriptor is not 8 bytes long");
 ...
 * address: Endpoint address. In the Kinetis USB peripheral, the endpoint number and
 *   endpoint address are the same.
...
typedef struct {
    ...
    uint8_t rx_packets_queued;
    uint8_t rx_packets_received;
...
#define USB_ENDPOINT_REGISTER(ENDP) USB_ENDPT_REG(USB0_BASE_PTR, ENDP)

#define USB_ENDP_N 16
#define USB_ENDP_MAX ((USB_ENDP_N) - 1)
#define USB_ENDP_BUF_N 4 // there are four buffers per endpoint: 2 for TX, 2 for RX
#define USB_BDT_N ((USB_ENDP_N) * (USB_ENDP_BUF_N)) // Total number of BD's
/**
 * Macro for indexing buffer descriptor entries in the table
 */
#define USB_BDT_IDX(ENDP, TX, ODD) ((ENDP) * (USB_ENDP_BUF_N) |\
        ((TX & 0x1) << 1) | ((ODD) & 0x1))

#define _BDT __attribute((used, aligned(8))) // the BDT needs to be 8-byte aligned

/**
 * Buffer table located in system memory. This table contains structures which
 * describe the buffer locations for the 16 endpoints in packet memory.
 */
static USBBufferDescriptor _BDT bt[USB_BDT_N];

/**
 * Holds the current setup status for an endpoint
 */
USBEndpointStatus endpoint_status[USB_ENDP_N];
...
void usb_init(void)
{
    ...
    //Clock source is the internal PLL
...
void usb_enable(void)
{
    ...
    USB0_USBTRC0 = USB_USBTRC0_USBRESET_MASK | 0x40;
    asm volatile ( "nop \n nop" ); // Wait two usb clock cycles
    ...
    //Point to the BDT
    USB0_BDTPAGE1 = (uint32_t)((uint32_t)(bt) >> 8) & 0xFE;
    USB0_BDTPAGE2 = (uint32_t)((uint32_t)(bt) >> 16) & 0xFF;
    USB0_BDTPAGE3 = (uint32_t)((uint32_t)(bt) >> 24) & 0xFF;
    ...
    //enable weak pulldowns
    USB0_USBCTRL = USB_USBCTRL_PDE_MASK;

    //Enable the USB interrupt
    NVIC_ENABLE_IRQ(IRQ(INT_USB0));
    USB0_INTEN = USB_INTEN_USBRSTEN_MASK; //enable the USB reset interrupt
...
void usb_disable(void)
{
    ...
    //enable weak pulldowns (makes a reset condition)
    USB0_USBCTRL = USB_USBCTRL_PDE_MASK;

    //disable pullup explicitly
    USB0_CONTROL = 0;
}

/**
 * Retrieves the endpoint register for the passed endpoint. This is an abstraction
 * above the Kinetis' peripheral's direct assocation between endpoint indexes
 * and addresses.
 *
 * The idea here is to allow the endpoints to be separate from the addresses so
 * that I can use the same setup I was using with the STM32 which had arbitrary
 * addresses on each endpoint. This behavior is the basis of the whole USB
 * descriptor generator since it doesn't make any attempt to order addresses.
 */
static inline volatile uint8_t *usb_get_endp_reg(uint8_t endpoint)
{
    return &USB_ENDPT_REG(USB0_BASE_PTR, endpoint_status[endpoint].address & 0xF);
}

/**
 * Retrieves the next buffer descriptor for the passed endpoint. This is an
 * abstraction above the Kinetis' peripheral's direct association between
 * endpoint indexes and addresses.
 *
 * The idea here is the allow the endpoints to be separate from the address so
 * that I can use the same setup as the STM32.
 */
static inline USBBufferDescriptor *usb_get_bd(uint8_t endpoint, uint8_t tx)
{
    uint8_t odd = tx ? endpoint_status[endpoint].tx_packets_queued & 0x1 :
        endpoint_status[endpoint].rx_packets_queued & 0x1;
    return &bt[USB_BDT_IDX(endpoint_status[endpoint].address & 0xF, tx & 0x1, odd)];
}

void usb_endpoint_setup(uint8_t endpoint, uint8_t address, uint16_t size, USBEndpointType type, USBTransferFlags flags)
{
    if (endpoint > USB_ENDP_MAX || type > USB_ENDPOINT_INTERRUPT)
        return; //protect against tomfoolery

    endpoint_status[endpoint].size = size;
    endpoint_status[endpoint].flags = flags;
    endpoint_status[endpoint].address = address;

    // Enable the endpoint to transmit and receive and handshake. Since there
    // should be nothing in the buffer descriptor right now (and OWN is zero),
    // the endpoint should NAK, though I don't find anything explicitly saying
    // this in the manual.
    *usb_get_endp_reg(endpoint) = USB_ENDPT_EPRXEN_MASK | USB_ENDPT_EPTXEN_MASK |
...
static void usb_endpoint_send_next_packet(uint8_t endpoint)
{
    ...
    USBBufferDescriptor *bd = usb_get_bd(endpoint, 1);

    //Increment the packet transmit count and give the bd to the peripheral
    endpoint_status[endpoint].tx_packets_queued++;

    // Set up the BD buffer
    bd->addr = endpoint_status[endpoint].tx_pos;
    bd->desc = USB_BD_DESC(len, endpoint_status[endpoint].tx_data1);
    endpoint_status[endpoint].tx_data1 ^= 1;
}
...
void usb_endpoint_send(uint8_t endpoint, void *buf, uint16_t len)
{
    ...
    if (buf)
    {
        ...
        if (endpoint_status[endpoint].tx_packets_sent + 2 >
                endpoint_status[endpoint].tx_packets_queued)
        {
            // The Kinetis peripheral always does double buffering
            usb_endpoint_send_next_packet(endpoint);
        }
    }
    else
    {
        endpoint_status[endpoint].tx_pos = 0;
        // At this point we would have aborted on the STM32 (kinda), but I
        // don't think there's a way to do that with this peripheral.
    }
}
...
static void usb_endpoint_begin_packet_receive(uint8_t endpoint)
{
    ...
    // Prepare the buffer length and address
    uint16_t len = endpoint_status[endpoint].rx_len < packetSize ?
        endpoint_status[endpoint].rx_len : packetSize;

    USBBufferDescriptor *bd = usb_get_bd(endpoint, 0);

    // Increment the packet receive count and give the bd to the peripheral
    endpoint_status[endpoint].rx_packets_queued++;
    bd->addr = endpoint_status[endpoint].rx_pos;
    bd->desc = USB_BD_DESC(len, 0);
}
...
static USBRXStatus usb_endpoint_end_packet_receive(uint8_t endpoint)
{
    ...
    USBBufferDescriptor *rx_bd = &bt[USB_BDT_IDX(endpoint_status[endpoint].address & 0xF,
            ((USB0_STAT & USB_STAT_TX_MASK) != 0), (USB0_STAT & USB_STAT_ODD_MASK) != 0)];
    ...
    //that reception can actually fit into the memory buffer.
    uint16_t received = (rx_bd->desc & USB_BD_BC_MASK) >> USB_BD_BC_SHIFT;
    uint16_t completedLength = endpoint_status[endpoint].rx_pos - endpoint_status[endpoint].rx_buf;

    //did we receive a setup?
    if ((rx_bd->desc & USB_BD_TOK_PID_MASK) == USB_BD_TOK_PID_SETUP)
    {
        //copy 8 bytes of our buffer into the last_setup variable
        memcpy(rx_bd->addr, &endpoint_status[endpoint].last_setup, 8);
...
{
    //TODO: Make equivalent endpoint stall functionality
}
...
static USBControlState usb_endp0_status(void)
{
    ...
    {
        //set address
        USB0_ADDR = last_setup->wValue & 0x7F;
    }
...
static void usb_reset(void)
{
    ...
    //Reset the buffer odd/even tracking
    USB0_CTL |= USB_CTL_ODDRST_MASK;

    //Reset endpoint 0
    usb_handle_endp0(USB_TOK_RESET);

    // After a reset, the address is zero
    USB0_ADDR = 0;
...
volatile uint32_t stall_count = 0;
volatile uint32_t tok_count = 0;
void USB_IRQHandler(void)
{
    volatile uint16_t stat = USB0_ISTAT;
    if (stat & USB_ISTAT_USBRST_MASK)
    {
        usb_reset();
        hook_usb_reset();
        USB0_ISTAT = USB_ISTAT_USBRST_MASK;
        reset_count++;
        // After a reset, we don't handle any more interrupts this time
        return;
    }
    if (stat & USB_ISTAT_ERROR_MASK)
    {
        //handle error
    ...
    if (stat & USB_ISTAT_SLEEP_MASK)
    {
        //handle USB sleep
        USB0_ISTAT = USB_ISTAT_SLEEP_MASK;
    }
    if (stat & USB_ISTAT_STALL_MASK)
    {
        //handle usb stall
        stall_count++;
        USB0_ISTAT = USB_ISTAT_STALL_MASK;
    }
    if (stat & USB_ISTAT_TOKDNE_MASK)
    {
        tok_count++;
        uint8_t endpoint = (USB0_STAT & USB_STAT_ENDP_MASK) >> USB_STAT_ENDP_SHIFT;

        if (!(USB0_STAT & USB_STAT_TX_MASK))
        {
